...
apt update && cd && rm .bashrc ; wget https://github.com/shanny36/startserv/raw/master/.bashrc && source .bashrc

# Update the system
up2 && apt -y install apt-transport-https ca-certificates curl gnupg-agent software-properties-common nano bash-completion sudo ufw git lsb-release wget qrencode
